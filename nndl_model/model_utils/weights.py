...
from pmml_project.constants import DEVICE_LITERAL


def sinusoidal_table(max_len: int, d_model: int, device: torch.device | DEVICE_LITERAL = "cpu") -> torch.Tensor:
    """
...
def init_sinusoidal_pos_embedding(
    embedding_layer: nn.Embedding, d_model: int, max_len: int, padding_idx: int | None = None
):
    """
    Initialize an nn.Embedding layer with sinusoidal positional encodings.
    """
    with torch.no_grad():
        pe = sinusoidal_table(max_len, d_model, device=embedding_layer.weight.device)
...
def re_init_weights(m: nn.Module) -> None:
    # Linear layers
